classdef AdaptiveOptopatchController < handle
    ...
    methods
        ...
        function preview=waveformPreview(controller,options)
            ...
            first=plan.manifest.trials.pulse_schedule{1};
            has1p=any(first.events.stimulation_source=="1p_dmd");
            has2p=any(first.events.stimulation_source=="2p_spiral");
            if has1p&&has2p, preview.mode="mixed";
            elseif has2p, preview.mode="2p_spiral";
            elseif has1p, preview.mode="1p_dmd"; else, preview.mode="none"; end
            if has2p
                preview=fill_2p_waveform_preview(preview, ...
                    controller.build2pPreviewWaveforms(plan),options.MaximumPoints);
                preview.acquisition_id=string(first.acquisition_id);
                if has1p, preview=add_1p_channels_to_dense_preview(preview,first,plan.targets); end
            elseif has1p
                preview=fill_1p_waveform_preview(preview,plan, ...
                    options.MaximumPoints);
            else
                preview.message="The first acquisition contains only null events.";
            end
            preview.available=true;
        end
        ...
        function report=preflightPlan(controller,plan)
            ...
            has1p=any(plan.manifest.trials.onephoton_event_count>0);
            has2p=any(plan.manifest.trials.twophoton_event_count>0);
            oneHardware=struct([]); twoHardware=struct([]);
            if has2p
                bundleReport=adaptive_optopatch.validate_2p_planning_bundle(plan.targets);
                issues=[issues;bundleReport.issues(:)];
            end
            if isempty(issues) && has1p
                oneHardware=adaptive_optopatch.resolve_luminos_1p_hardware(controller.LuminosApp);
            end
            if isempty(issues) && has2p
                twoHardware=adaptive_optopatch.resolve_luminos_2p_hardware( ...
                    controller.LuminosApp,"ApplyCalibration",false);
            end
            if isempty(issues)
                if has1p, hardware=oneHardware; else, hardware=twoHardware; end
                adaptive_optopatch.validate_camera_geometry(hardware.voltage_camera,plan.targets);
                for rowIndex=1:height(plan.manifest.trials)
                    resolved=plan.manifest.trials.pulse_schedule{rowIndex};
                    oneRows=resolved.events.stimulation_source=="1p_dmd";
                    twoRows=resolved.events.stimulation_source=="2p_spiral";
                    sequencePlan=struct([]); waveforms=struct([]);
                    if any(oneRows)
                    ...
                    if any(twoRows)
                        index=unique(resolved.events.target_index(twoRows));
                        target=plan.targets.targets(index(1));
                        waveforms=adaptive_optopatch.build_2p_trial_waveforms( ...
                            resolved,target,plan_targeting_transform(plan), ...
                            "SampleRateHz",double(twoHardware.daq.global_props.rate), ...
                    ...
                    [globalProps,wfmData]=adaptive_optopatch.build_luminos_mixed_waveform_config( ...
                        hardware.daq.global_props,hardware.daq.wfm_data,resolved, ...
                        "DmdSequencePlan",sequencePlan,"TwoPhotonWaveforms",waveforms);
                    if any(oneRows)&&any(twoRows), modality="mixed";
                    elseif any(twoRows), modality="2p_spiral"; else, modality="1p_dmd"; end
                    accounting=adaptive_optopatch.account_stimulation_outputs( ...
                        globalProps,wfmData,"Modality",modality, ...
                    ...
                    preflight_camera_frames(hardware.cameras,globalProps.total_time, ...
                        any(twoRows)&&parameters.allow_camera_rate_override);
                end
            end
            ...
            report=struct("schema_version","0.2.0","passed",isempty(issues), ...
                ...
                "mode","event_sources","issues",issues);
        ...
        function waveforms=build2pPreviewWaveforms(controller,plan)
            ...
            hardware=adaptive_optopatch.resolve_luminos_2p_hardware( ...
                controller.LuminosApp,"ApplyCalibration",false);
            rowIndex=find(plan.manifest.trials.twophoton_event_count>0,1);
            protocol=plan.manifest.trials.pulse_schedule{rowIndex};
            index=unique(protocol.events.target_index( ...
                protocol.events.stimulation_source=="2p_spiral"));
            target=plan.targets.targets(index(1));
        ...
        function summary=planSummary(controller)
            ...
            summary=struct("schema_version","1.0.0","prepared",false, ...
                ...
                "onephoton_event_count",0,"twophoton_event_count",0, ...
                "onephoton_acquisition_count",0,"twophoton_acquisition_count",0, ...
                "mixed_acquisition_count",0, ...
            ...
            summary.onephoton_event_count=sum(trials.onephoton_event_count);
            summary.twophoton_event_count=sum(trials.twophoton_event_count);
            summary.onephoton_acquisition_count=sum(trials.onephoton_event_count>0);
            summary.twophoton_acquisition_count=sum(trials.twophoton_event_count>0);
            summary.mixed_acquisition_count=sum(trials.has_mixed_sources);
        end
            ...
            if has2p
                if ~isfield(plan.reference,"scanner") || ...
                    ...
                    error("adaptive_optopatch:FrozenScannerCalibrationMissing", ...
                        "The frozen reference model does not contain a scanner targeting calibration.");
                end
                scanner=plan.reference.scanner;
            end
            run=adaptive_optopatch.run_mixed_manifest( ...
                plan.manifest,plan.targets,controller.LuminosApp, ...
                "ReleaseLevel","standard", ...
                "OutputDirectory",controller.ActiveRunFolder, ...
                "OutputRoot",outputRoot,"Resume",true,"StopAfterTrial",count, ...
...
event=struct("cell_id","","onset_s",NaN,"offset_s",NaN, ...
    "command_voltage_v",NaN,"is_null",false,"stimulation_source","");
end
...
for k=1:numel(waveforms.per_pulse)
    ...
    event.stimulation_source="2p_spiral";
...
for k=1:height(pulses)
    ...
    event.stimulation_source=string(pulses.stimulation_source(k));
    preview.events(end+1,1)=event;
end
...
function preview=add_1p_channels_to_dense_preview(preview,protocol,targets)
...
for k=1:height(pulses)
    ...
    event.offset_s=double(pulses.offset_s(k));
    event.command_voltage_v=double(pulses.command_voltage_v(k));
    event.stimulation_source="1p_dmd";
    preview.events(end+1,1)=event;
end
...
resolved=adaptive_optopatch.resolve_protocol(plan.protocol_definition, ...
    plan.fov_state,plan.targets,defaults, ...
