...
if isempty(fieldnames(options.FovState))
    error("adaptive_optopatch:FovStateRequired", ...
        "Schema-4 protocol resolution requires the current FOV state.");
end
resolvedProtocols=adaptive_optopatch.resolve_protocol(definition, ...
    options.FovState,targets,options.GuiDefaults);
...
for k=1:n
    ...
    sources=unique(events.stimulation_source(events.stimulation_source~="none"),"stable");
    onephotonEventCount=sum(events.stimulation_source=="1p_dmd");
    twophotonEventCount=sum(events.stimulation_source=="2p_spiral");
    hasMixedSources=onephotonEventCount>0 && twophotonEventCount>0;
    if hasMixedSources, mode="mixed"; elseif twophotonEventCount>0, mode="2p_spiral";
    elseif onephotonEventCount>0, mode="1p_dmd"; else, mode="none"; end
    stimulationSources={sources};
    trialId=k; repeatIndex=1; isNull=all(events.is_null);
    ...
    row=table(trialId,repeatIndex,mode,stimulationSources,onephotonEventCount, ...
        twophotonEventCount,hasMixedSources,targetCellId,isNull,targetIndex, ...
        ...
        'stimulation_sources','onephoton_event_count','twophoton_event_count', ...
        'has_mixed_sources', ...
