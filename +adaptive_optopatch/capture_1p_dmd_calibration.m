...
entry=struct("name","","present",false,"has_transform",false, ...
    ...
    "reference_world_limits",[],"reference_roi",[],"reference_bin",NaN, ...
...
[entry.reference_image_size,entry.reference_world_limits, ...
    entry.reference_bin,entry.reference_identifier]=reference_geometry( ...
    refimage,read_member(dmd,"refimagefile",""));
if numel(entry.reference_world_limits)==4
    entry.reference_roi=[entry.reference_world_limits(1), ...
        diff(entry.reference_world_limits(1:2)), ...
        entry.reference_world_limits(3), ...
        diff(entry.reference_world_limits(3:4))];
end
...
function [imageSize,worldLimits,bin,identifier]=reference_geometry(refimage,path)
imageSize=[]; worldLimits=[]; bin=NaN; identifier=string(path);
...
bin=double(read_member(refimage,"bin",NaN));
if strlength(identifier)==0
    identifier=string(read_member(refimage,"name",""));
end
