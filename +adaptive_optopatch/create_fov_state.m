...
arguments
    ...
    roiPolygons cell = {}
    options.StimulationMode (1,1) string {mustBeMember(options.StimulationMode,["1p_dmd","2p_spiral"])} = "2p_spiral"
    options.MicronsPerPixel (1,1) double = NaN
...
if ~isfinite(micronsPerPixel), micronsPerPixel=double(reference.microns_per_pixel); end
if ~isfinite(micronsPerPixel) || micronsPerPixel<=0
    error("adaptive_optopatch:InvalidMicronsPerPixel", ...
        "FOV microns_per_pixel must be positive and finite.");
end
reference=ensure_cell_state(reference,roiPolygons);
fovState=struct("schema_version","2.0.0", ...
    ...
    "stimulation_mode",options.StimulationMode, ...
