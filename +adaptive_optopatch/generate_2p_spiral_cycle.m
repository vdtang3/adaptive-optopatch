...
galvoTheta=unwrap(atan2(outboundV(:,2)-centerV(2), ...
    outboundV(:,1)-centerV(1)));
angleChange=galvoTheta(end)-galvoTheta(find(hypot( ...
    outboundV(:,1)-centerV(1),outboundV(:,2)-centerV(2))>eps,1));
if angleChange>=0, rotationDirection="increasing";
else, rotationDirection="decreasing";
end
cycle=struct("schema_version","0.1.0","x_v",x,"y_v",y, ...
    ...
    "cycle_samples",numel(x),"galvo_angle_direction",rotationDirection);
end
