...
for k=1:numel(protocol.acquisitions)
    ...
    % form. An acquisition with no scheduler field is an ordinary explicit
    % schedule and is untouched. See acquisition_scheduler_spec.
    scheduler=adaptive_optopatch.acquisition_scheduler_spec(acquisition);
    if ~isempty(scheduler)
        acquisition.scheduler=scheduler;
        if height(acquisition.events)~=1
            error("adaptive_optopatch:InvalidAcquisitionScheduler", ...
                "A scheduler-backed acquisition carries exactly one " + ...
                "untargeted TEMPLATE event, which supplies the event tier " + ...
                "of parameter resolution. Acquisition %s has %d. The " + ...
                "pulses themselves are realized against the selected " + ...
                "cells at Update plan.", ...
                string(acquisition.acquisition_id),height(acquisition.events));
        end
        if ismember("target_cell_id", ...
                string(acquisition.events.Properties.VariableNames))
            error("adaptive_optopatch:InvalidAcquisitionScheduler", ...
                "A scheduler-backed acquisition must name no target. " + ...
                "Acquisition %s carries target_cell_id on its template " + ...
                "event; its targets are whichever cells are Stim-enabled " + ...
                "at Update plan.",string(acquisition.acquisition_id));
        end
    end
