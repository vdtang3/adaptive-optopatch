...
hardware.blue_dmd=app.getDevice("DMD","name",profile.inactive_one_photon.dmd.name);
hardware.blue_shutter=app.getDevice("NI_DAQ_Shutter", ...
    "name",profile.inactive_one_photon.shutter.name);
hardware.cameras=app.getDevice("Camera");
if numel(hardware.daq)~=1 || numel(hardware.scanner)~=1 || ...
        numel(hardware.modulator)~=1 || numel(hardware.blue_dmd)~=1 || ...
        numel(hardware.blue_shutter)~=1
    error("adaptive_optopatch:TwoPhotonHardwareMissing", ...
        ["The combined DAQ, Chameleon scanner, 2P modulator, or Blue DMD " + ...
         "or 488 shutter was not found uniquely."]);
end
