...
| `create_single_cell_ramp_protocol.m` | Ascending arbitrary Blue-voltage blocks for one cell |
| `create_round_robin_protocol.m` | Randomized interleaving using calibrations from a saved FOV |
...
Every `protocol.events` row is one physical pulse. A finite
`command_voltage_v` is used directly and takes precedence. Otherwise,
`amplitude_fraction` is relative to the voltage selected and frozen in the
acquisition GUI. Null pulses are explicitly marked with `is_null` and resolve
to zero output.
