classdef TestAdaptiveOptopatch < matlab.unittest.TestCase
    methods (Test)
        ...
        function enforcesStagedTwoPhotonReleaseLevels(testCase)
            ...
            pilotProtocol=resolve_for_test(adaptive_optopatch.generate_screen_protocol( ...
            ...
            unlimitedProtocol=resolve_for_test(adaptive_optopatch.generate_screen_protocol( ...
        ...
        function flattensScreenAndStfPulseSchedules(testCase)
            ...
            screen=resolve_for_test(screen,"1p_dmd");
            ...
            stf=resolve_for_test(stf,"1p_dmd");
        ...
        function runsSimulatedOnePhotonManifest(testCase)
            ...
            targets=struct("targets",target,"dmd_camera_masks",true(8,9,1), ...
                ...
                "reference_camera",frame_camera_geometry([8 9]), ...
        ...
        function rejectsInvalidAndObsoleteTrainProtocols(testCase)
            ...
            testCase.verifyFalse(report.passed);
            obsolete=invalid; obsolete.schema_version="2.0.0";
        ...
        function mapsRelativeAmplitudeToPhysicalHardwareVoltage(testCase)
            ...
            protocol=resolve_for_test(definition,"1p_dmd");
        ...
        function frozenTwoPhotonRunIgnoresLaterActiveCalibrationChange(testCase)
            ...
            profile=adaptive_optopatch.virtual_upright_2p_profile();
            calibrationB=distinct_galvo_calibration(profile,"CALIBRATION_B", ...
        ...
        function roundRobinUsesOnlyCalibratedEnabledCells(testCase)
            ...
            first=adaptive_optopatch.resolve_protocol(firstDefinition,fovState, ...
                targets,test_gui_defaults(),"Mode","1p_dmd"); first=first{1};
            second=adaptive_optopatch.resolve_protocol(secondDefinition,fovState, ...
                targets,test_gui_defaults(),"Mode","1p_dmd"); second=second{1};
        ...
        function validatesPulseIdentityTargetsAndPhysicalVoltage(testCase)
            ...
            resolved=resolve_for_test(protocol,"1p_dmd");
        ...
        function calibrationDriftIsAdvisoryButUnsafeStatesStillError(testCase)
            ...
            fovState=adaptive_optopatch.update_cell_eligibility(fovState, ...
                "cell_002","StimulationEnabled",false);
            fovState=adaptive_optopatch.update_cell_eligibility(fovState, ...
                "cell_003","StimulationEnabled",false);
            ...
            targets=adaptive_optopatch.build_target_bundle(fovState.reference, ...
                ...
                "BlueMaskAdjustmentPixels",0);
            manifest=adaptive_optopatch.build_manifest(fovState.reference,targets, ...
                definition,"Mode","1p_dmd","CurrentObisPowerW",0.02, ...
                "FovState",fovState,"GuiDefaults",test_gui_defaults());
            ...
            testCase.verifyTrue(all(ismember(["blue_mask_adjustment_changed", ...
                ...
                "obis_setpoint_changed","selected_voltage_changed"],codes)));
            testCase.verifyFalse(any(contains(codes,"orange")));
            testCase.verifyEqual(manifest.trials.pulse_schedule{1}.events.command_voltage_v,0.9);
            ...
            disabledFov.reference.cells=disabledFov.cells;
            testCase.verifyError(@()adaptive_optopatch.build_manifest( ...
                disabledReference,disabledTargets,definition,"Mode","1p_dmd", ...
                "FovState",disabledFov,"GuiDefaults",test_gui_defaults()), ...
...
function artifact=distinct_galvo_calibration(profile,calibrationId,tform)
...
function roi=unified_camera_roi()
% Sensor ROI [left width top height] matching unified_test_info's 80x100
% reference grid, so the simulated Camera 1 acquires on the grid the frozen
% targets are expressed in.
roi=[974 100 984 80];
end

function geometry=frame_camera_geometry(imageSize)
% Frozen camera geometry for a synthetic reference of the given size at the
% sensor origin with no binning.
