classdef TestDmdCameraMaskRemapping < matlab.unittest.TestCase
    methods (Test)
        ...
        function roundRobinStackIsMappedWithoutReordering(testCase)
            ...
            testCase.verifyTrue(dmd.slot_patterns{1}(13,11));
            testCase.verifyTrue(dmd.slot_patterns{2}(18,16));
            testCase.verifyFalse(any(dmd.slot_patterns{1} & ...
                dmd.slot_patterns{2},"all"));
        end
...
plan=struct("schema_version","2.0.0","unique_camera_masks",stack, ...
    "reference_camera",camera,"pattern_count",2,"event_count",2, ...
    "unique_mask_count",2,"pulse_id",[1;2],"event_slot_indices",[1;2], ...
